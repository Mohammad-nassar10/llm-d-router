...
| `latency-observer-producer` | [`latencyobserver`](latencyobserver/) | `TTFTPercentiles` | Measures each request's actual time-to-first-token and publishes per-endpoint percentile anchors for `latency-observation-scorer`. Needs nothing from the endpoint but its response. |
...
- `latency-observer-producer` **requires** `InFlightLoad`, so `inflight-load-producer` is ordered ahead of it and auto-created when absent. It is itself auto-created by `latency-observation-scorer`, so configuring that scorer alone wires the whole chain.
